...
// users can trigger it — the recipient list and content are always built
// server-side-adjacent from real workflow data in app.js, never from free-form
// user input, so this isn't an open relay despite having no per-recipient
// allowlist here.
const { getAdminApp } = require('./_firebaseAdmin');

function buildEmailHtml({ heading, message, ctaLabel, ctaUrl }) {
    const ctaBlock = ctaUrl ? `<div style="text-align: center; margin: 0 0 28px;">
      <a href="${ctaUrl}" style="display: inline-block; background-color: #14B8A6; color: #ffffff; text-decoration: none; font-weight: 700; font-size: 13px; letter-spacing: 0.5px; padding: 15px 40px; border-radius: 10px;">${ctaLabel || 'OPEN PORTAL'}</a>
    </div>` : '';
  ...
  <div style="padding: 36px 32px;">
    <h2 style="color: #0B1E36; font-size: 19px; margin: 0 0 20px; text-align: center;">${heading}</h2>
    <p style="color: #475569; font-size: 14px; line-height: 1.7; margin: 0 0 28px; white-space: pre-wrap;">${message}</p>
    ${ctaBlock}
  </div>
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (!recipients.length) { res.status(400).json({ error: 'At least one valid recipient email is required.' }); return; }
